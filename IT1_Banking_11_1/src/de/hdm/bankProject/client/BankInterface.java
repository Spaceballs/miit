...
public class BankInterface implements ActionListener, TreeSelectionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().equals("depositAmount")) {
            if (selectedAccount != null) {
                ...
                try {
                    ...
                    verwaltung.bookToAccount(selectedAccount, tmpValue, kof.getBookingText());
        ...
        if (e.getActionCommand().equals("createCurrencyAccount")) {
            if (selectedCustomer != null) {
                //System.out.println("Creating Account for " + kuf.getVorname() + " " + kuf.getNachname() + ".");
                JComboBox currencyComboBox = (JComboBox)e.getSource();
                setSelectedAccount(
                        verwaltung.createCurrencyAccountFor(
                        selectedCustomer, currencyComboBox.getSelectedIndex()));
                kuk.addAccountTreeNode(new AccountInfo(selectedAccount));
            } else {
                JOptionPane.showMessageDialog(frame, "Kein Kunde ausgewählt!");
            }
        ...
        if (e.getActionCommand().equals("getAccountStatement")) {
            if (selectedAccount != null) {
                JDialog accountDialog = new JDialog();
                JTextArea statementTextArea = new JTextArea(selectedAccount.getAccountStatement());
                statementTextArea.setFont(Font.decode("Monospaced"));
                accountDialog.add(statementTextArea);
                accountDialog.pack();
        ...
        if (e.getActionCommand().equals("interest")) {
            if (selectedAccount != null) {
                double interest = selectedAccount.getInterest();
                JOptionPane.showMessageDialog(frame, "Der Jahreszins beträgt " +
                        new DecimalFormat("#0.00").format(interest) + " Euro");
            } else {
                JOptionPane.showMessageDialog(frame, "Kein Konto ausgewählt!");
            }
        }

        if (e.getActionCommand().equals("updateBalance")) {
            if (selectedAccount != null) {
                verwaltung.updateAccountBalance(selectedAccount);
                JOptionPane.showMessageDialog(frame, "Der Kontostand wurde aktualisiert");
            } else {
                JOptionPane.showMessageDialog(frame, "Kein Konto ausgewählt!");
            }
        }

        if (e.getActionCommand().equals("newCreditLine")) {
            if (selectedAccount != null) {
                double tmpValue;
                try {
                    tmpValue = kof.getCreditLine();
                    //System.out.println("Crditline is now: " + tmpValue + ".");
                    verwaltung.changeCreditLine(selectedAccount, tmpValue);
                    kof.updateCreditLine(selectedAccount);
                    kof.updateBalance(selectedAccount);
                } catch (ParseException pe) {
                    JOptionPane.showMessageDialog(frame, "Kein korrekter Betrag angegeben!");
                }
        ...
        if (e.getActionCommand().equals("cloneAccount")) {
            if (selectedAccount != null) {
                //System.out.println("Creating Account for " + kuf.getVorname() + " " + kuf.getNachname() + ".");
                setSelectedAccount(verwaltung.cloneAccount(selectedAccount));
                kuk.addAccountTreeNode(new AccountInfo(selectedAccount));
            } else {
                JOptionPane.showMessageDialog(frame, "Kein Konto ausgewählt!");
            }
    ...
    protected void generateMenus(JMenuBar menubar) {
        ...
        tmpMenuItem = new JMenuItem("Alle Konten ausgeben");
        tmpMenuItem.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                verwaltung.getBank().printAccounts();
            }
        });
        aktionenMenu.add(tmpMenuItem);
        ...
        tmpMenuItem = new JMenuItem("Betrag einzahlen");
        tmpMenuItem.setActionCommand("depositAmount");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Betrag abheben");
        tmpMenuItem.setActionCommand("withdrawAmount");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Neues Konto erzeugen");
        tmpMenuItem.setActionCommand("createAccount");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Konto löschen");
        tmpMenuItem.setActionCommand("deleteAccount");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Zins berechnen");
        tmpMenuItem.setActionCommand("interest");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Kontoauszug anzeigen");
        tmpMenuItem.setActionCommand("getAccountStatement");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Kontostand aktualisieren");
        tmpMenuItem.setActionCommand("updateBalance");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);

        tmpMenuItem = new JMenuItem("Konto clonen");
        tmpMenuItem.setActionCommand("cloneAccount");
        tmpMenuItem.addActionListener(this);
        kontoMenu.add(tmpMenuItem);
        ...
        /**
         * Menü für die Übungen zur Überprüfung einzelner Teilaufgaben
         */
        JMenu practiceMenu = new JMenu("Übungen");
        menubar.add(practiceMenu);

        tmpMenuItem = new JMenuItem("Namenslänge");
        tmpMenuItem.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent ae) {
                if (selectedCustomer != null) {
                    JOptionPane.showMessageDialog(
                            frame, "Die Länge des Kundennamens von " + selectedCustomer.toString() + " ist " + selectedCustomer.nameLength());
                } else {
                    JOptionPane.showMessageDialog(frame, "Kein Kunde ausgewählt!");
                }
